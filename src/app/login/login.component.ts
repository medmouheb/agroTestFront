...
export class LoginComponent implements OnInit {
  ...
  currentStage: string = "login"

  changeCurrentStage(s: string) {
    this.currentStage = s
  }
  ...
  onClick() {
    this.loginService.signIn(this.username, this.password).subscribe(
      (res: any) => {
        localStorage.setItem("tocken",  JSON.stringify( res));
  ...
  selectContry(c: any) {
    this.user.numeroTelephone = c.target.value
  ...
  phoneNumberinp(e: any) {
    ...
    if (this.user.numeroTelephone.toString().length != 12) {
      this.minIphone = true;
    } else {
  ...
  invalidPasword: boolean = false
  invalidConfirmPasword: boolean = false

  signUp() {

    if (!this.user.email || !this.user.nom || !this.user.prenom || !this.user.username || !this.user.dateNaissance || !this.user.sexe || !this.user.pays || !this.user.region || !this.user.numeroTelephone || !this.user.password || !this.user.confirmPassword) {
      this.emptyFields = true
    } else {
      this.emptyFields = false
    }

    if (this.user.password.length < 8 || this.user.password.length > 20 || !/^[a-zA-Z0-9]+$/.test(this.user.password) || !/[A-Z]/.test(this.user.password)) {
      this.invalidPasword = true
    } else {
      this.invalidPasword = false
    }

    if (this.user.password != this.user.confirmPassword) {
      this.invalidConfirmPasword = true
    } else {
      this.invalidConfirmPasword = false
    }

    if (!this.emptyFields && !this.invalidPasword && !this.invalidConfirmPasword) {
      this.loginService.signUp(this.user).subscribe((data) => {

        this.changeCurrentStage("login")
        this.toastService.close("0");
        this.toastService.success(
          this.translateService.instant("successful SignUp", {
            elem: this.translateService.instant("SignUp"),
          })
        );
        ...
        setTimeout(() => {
          this.toastService.close("0");
          this.toastService.success(
            this.translateService.instant("Please continue to LogIn", {
              elem: this.translateService.instant("LogIn"),
            })
          );
        }, 1000);
        ...
        this.toastService.error(
          this.translateService.instant("SignUp failed" + error.error.message, {
            elem: this.translateService.instant(error.error.message),
          })
        );
      })
    }
