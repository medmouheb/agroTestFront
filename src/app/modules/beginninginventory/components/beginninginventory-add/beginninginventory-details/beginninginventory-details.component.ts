...
export class BeginninginventoryDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private compaser: BeginninginventoryService,
  ) {}
  codes: Array<String> = [];
  names: Array<String> = [];
  static = "";
  ...
  ngOnInit(): void {
    ...
    if (this.camp == undefined) {
      this.camp = { nomDuProduit: "", codeProduit: "" };
    }
    this.initForm();
  ...
  initForm() {
    this.addform = this.fb.group({
      ...
      nomDuProduit: [null, Validators.required],
    });
  }
  ...
  isBlur() {
    ...
    } else if (this.camp.codeProduit.toString().length < 1) {
      this.minIstrueCode = true;
    } else {
