...
export class SimulatorResultsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    localStorage.getItem("simulator-product")).subscribe((res : any) => {
      for (let index = 0; index < res.length; index++) {
        const element = res[index];
        const numberOfOperations = element.operations ? Object.keys(element.operations).length : 0;
        this.cards.push({ id: index + 1, name: element.product, flag: numberOfOperations, people: 99 });
      }}
    )
  }
