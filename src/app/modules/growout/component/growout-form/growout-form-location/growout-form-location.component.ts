...
export class GrowoutFormLocationComponent implements OnInit {
  ...
  isBlur() {
    if (
      this.growout.wilayaName.toString().length <= 0 ||
      this.growout.wilayaName.toString().length > 100
    ) {
  ...
  isBlur2() {
    if (
      this.growout.wilayaCode.toString().length <= 0 ||
      this.growout.wilayaCode.toString().length > 3
    ) {
