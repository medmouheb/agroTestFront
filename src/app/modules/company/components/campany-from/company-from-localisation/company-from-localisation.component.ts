...
export class CompanyFromLocalisationComponent implements OnInit {
  ...
  isBlur() {
    if (
      this.camp.wilayaName.toString().length <= 0 ||
      this.camp.wilayaName.toString().length > 100
    ) {
  ...
  isBlur2() {
    if (
      this.camp.zipCode.toString().length <= 0 ||
      this.camp.zipCode.toString().length > 11
    ) {
  ...
  isBlur3() {
    if (this.camp.number.toString().length != 12) {
      this.minIphone = true;
      this.sharedService.setIsActive(false);
    } else {
