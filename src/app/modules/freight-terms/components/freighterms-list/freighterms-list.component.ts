...
export class FreightermsListComponent implements OnInit {
  ...
  onSave(id: string | null) {
    ...
    this.freighttermsservice
      .findbycode(this.freightterm.freighttermcode)
      .subscribe((data) => {
        if (data != null) {
          this.toastService.warning("Ship methode code must be unique");
          this.toastService.close("0");
          this.toastService.error(this.translateService.instant("freightterm"));
          return;
        }
  ...
  sortByCode() {
    if (this.affiche) {
      if (this.sortByCodeValid) {
        this.freighttermss.sort((a, b) =>
          a.freighttermcode.localeCompare(b.freighttermcode),
        );
        this.sortByCodeValid = false;
      } else {
        this.freighttermss.sort((a, b) =>
          b.freighttermcode.localeCompare(a.freighttermcode),
        );
        this.sortByCodeValid = true;
      }
    } else {
      if (this.sortByCodeValid) {
        this.freightterms.sort((a, b) =>
          a.freighttermcode.localeCompare(b.freighttermcode),
        );
        this.sortByCodeValid = false;
      } else {
  ...
  sortByName() {
    if (this.affiche) {
      if (this.sortByNameValid) {
        this.freighttermss.sort((a, b) =>
          a.freighttermname.localeCompare(b.freighttermname),
        );
        this.sortByNameValid = false;
      } else {
        this.freighttermss.sort((a, b) =>
          b.freighttermname.localeCompare(a.freighttermname),
        );
